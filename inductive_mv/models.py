...
class RED_GNN_induc(torch.nn.Module):
    ...
    def forward(self, subs, rels, mode='transductive', explain=False, train_mode='train'):
        # subs = subs[:1]
        # rels = rels[:1]
        n = len(subs)
        n_ent = self.loader.n_ent if mode=='transductive' else self.loader.n_ent_ind
        n_gene = self.loader.n_gene if mode=='transductive' else self.loader.n_gene_ind
        self.gene_idx = list(self.loader.entitypeid2geneid.keys()) if mode=='transductive' else list(self.loader.entitypeid2geneid_ind.keys())
        entity_pretrain_emb = self.loader.entity_pretrain_emb if mode=='transductive' else self.loader.entity_ind_pretrain_emb
