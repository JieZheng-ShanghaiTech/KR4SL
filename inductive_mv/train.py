...
def get_params():
    ...
    parser.add_argument('--data_path', type=str, default='./data/inductive/SL')
...
if __name__ == '__main__':
    ...
    opts.perf_file = os.path.join('results/'+suffix,  '_perf_'+str(args.time_num)+'fold_'+suffix+'.txt')
    opts.ckpt_file = os.path.join('results/'+suffix,  '_ckpt_model_'+str(args.time_num)+'fold_'+suffix+'.pkl')
    opts.best_1fold_file = os.path.join('results/'+suffix, '_best_'+str(args.time_num)+'fold_model_'+suffix+'.pkl')

    torch.cuda.set_device(args.gpu)
    print('gpu:', args.gpu)
   
    opts.lr = args.lr
    opts.lamb =0.0011  #0.000189, 0.002
    ...
    if opts.load_ckpt:
            ...
            best_p50, out_str_val, result_val = model.evaluate(data='valid')
            tst_p50, out_str_tst, best_result = model.evaluate(data='test')
    ...
    for epoch in range(start, 15):
        ...
        if (epoch + 1) % 3 == 0:
            i_time = time.time()
            train_mrr, out_str_tr, result_tr = model.evaluate(data='train')
            val_p50, out_str_val, result_val = model.evaluate(data='valid')
            tst_p50, out_str_tst, result_tst = model.evaluate(data='test')
